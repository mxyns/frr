...
{
	const char *names[19] = {
		"BGP_PATH_IGP_CHANGED",	  "BGP_PATH_DAMPED",
		"BGP_PATH_HISTORY",	  "BGP_PATH_SELECTED",
		"BGP_PATH_VALID",	  "BGP_PATH_ATTR_CHANGED",
		"BGP_PATH_DMED_CHECK",	  "BGP_PATH_DMED_SELECTED",
		"BGP_PATH_STALE",	  "BGP_PATH_REMOVED",
		"BGP_PATH_COUNTED",	  "BGP_PATH_MULTIPATH",
		"BGP_PATH_MULTIPATH_CHG", "BGP_PATH_RIB_ATTR_CHG",
		"BGP_PATH_ANNC_NH_SELF",  "BGP_PATH_LINK_BW_CHG",
		"BGP_PATH_ACCEPT_OWN",	  "BGP_PATH_BMP_LOCKED",
	...
	zlog_info("%s: bpi frmo peer=%pBP rx=%" PRIu32, __func__, bpi->peer,
		  bpi->addpath_rx_id);
...
static void bgp_show_mpath_info(struct bgp_path_info *bpi)
{
	...
	for (struct bgp_path_info_mpath *mpath = bpi->mpath; mpath;
	     mpath = mpath->mp_next) {
		zlog_info("%s: mp_count=%" PRIu16 ", mp_flags=%" PRIu16
			  ", mp_info=%p, mp_info_rx=%d",
			  __func__, mpath->mp_count, mpath->mp_flags,
			  mpath->mp_info,
			  (int)(mpath->mp_info ? mpath->mp_info->addpath_rx_id
					       : -1));
	...
	for (pi = bgp_dest_get_bgp_path_info(dest);
		...
		if (BGP_PATH_HOLDDOWN(pi)) {
			...
			if (CHECK_FLAG(pi->flags, BGP_PATH_REMOVED) &&
			    (pi != old_select)) {
				bgp_mpath_diff_insert(mpath_diff_list, pi, 0);
				 ...
				 afi_t afi, safi_t safi)
{
	...
	if (old_select)
		zlog_info("old_select %p, dest=%p, rx_id=%" PRIu32
			  ", peer=%pBP",
			  old_select, old_select->net,
			  old_select->addpath_rx_id, old_select->peer);
	else
		zlog_info("old_select null");

	if (new_select)
		zlog_info("new_select %p, dest=%p, rx_id=%" PRIu32
			  ", peer=%pBP",
			  new_select, new_select->net,
			  new_select->addpath_rx_id, new_select->peer);
	else
		zlog_info("new_select null");

	zlog_info("%s: multipath diff %p computed, mpath_changed=%d", __func__,
		  &mpath_diff, (int)bgp_mpath_diff_count(&mpath_diff));
	frr_each (bgp_mpath_diff, &mpath_diff, diff) {
		if (!diff->path)
			zlog_info("[%s] diff: %p ", diff->update ? "+" : "-",
				  diff);
		else {
			zlog_info(
