		...
		const char *string, uint16_t maxlen)
{
	...
	len = maxlen > 0 ? MIN(maxlen, len) : len;
	bmp_put_info_tlv_hdr(s, type, len);
...
bmp_put_info_tlv_vrftablename_with_index(struct stream *s, struct bgp *bgp,
	...
	if (vrftablename != NULL) {
		...
		len = MIN(len, BMP_INFO_LENGTH_TABLE_NAME_MAX);
 ...
 * if vrf is true this a peerstate for a vrf state
...
#define BGP_BMP_MAX_PACKET_SIZE	1024
	...
	if (!down && (peer_established(peer) || is_locrib)) {
	...
	if (is_locrib)
		bmp_put_info_tlv_vrftablename(s, peer->bgp);
...
bmp_monitor_rib_out_post_walk(struct bmp *bmp, afi_t afi, safi_t safi,
			      struct prefix *pfx, struct bgp_dest *dest,
...
static bool bmp_wrqueue_ribin(struct bmp *bmp, struct pullwr *pullwr)
{
	...
	bmp_monitor(bmp, peer, 0, bmp_get_peer_type(peer), &bqe->p, prd,
		    ...
		    adjin ? adjin->uptime : monotime(NULL), NULL, path_status);
...
static void bmp_wrfill(struct bmp *bmp, struct pullwr *pullwr)
{
	...
	case BMP_StartupIdle:
		...
		uint32_t micros_since =
			(uint32_t)(monotime_since(&bmp_startup_time, NULL));
		zlog_info(
 ...
 */
bool bmp_bgp_update_vrf_status(struct bmp_bgp *bmpbgp,
	...
	bmpbgp->vrf_up = force != vrf_state_unknown ? force :
			 vrf_is_enabled(vrf) ? vrf_state_up : vrf_state_down;
	...
	if (!post_policy) {
		...
		if (withdraw && !locked_path) {
			{ /* scope lock the vars declared by lookup */
				...
				if (!lbpi) {
					zlog_warn(
						"no locked path found for %pRN tx %" PRIu32" in bgp %s",
