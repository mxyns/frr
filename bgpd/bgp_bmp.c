...
static uint64_t bmp_get_peer_distinguisher(struct bmp* bmp, afi_t afi) {
	 ...
	 * vrf => has RD?
	 * 		if yes => use RD value
	 * 		else => use vrf_id and convert it so that peer_distinguisher is 0::vrf_id
	 */
	return bgp->inst_type == VRF_DEFAULT ? 0 : prd ? *(uint64_t*)prd->val : (((uint64_t)htonl(bgp->vrf_id)) << 32);
}
...
static void bmp_per_peer_hdr(struct stream *s, struct bgp* bgp, struct peer *peer,
		uint8_t flags, uint8_t peer_type_flag, uint64_t peer_distinguisher, const struct timeval *tv)
{
...
#define BGP_BMP_MAX_PACKET_SIZE	1024
	...
	if (peer_established(peer) && !down) {
		...
		bmp_per_peer_hdr(s, peer->bgp, peer, 0, BMP_PEER_TYPE_GLOBAL_INSTANCE, 0, &uptime_real);
...
static void bmp_wrmirror_lost(struct bmp *bmp, struct pullwr *pullwr)
{
	...
	bmp_per_peer_hdr(s, bmp->targets->bgp, bmp->targets->bgp->peer_self, 0, BMP_PEER_TYPE_GLOBAL_INSTANCE, 0, &tv);
...
static bool bmp_wrmirror(struct bmp *bmp, struct pullwr *pullwr)
{
	...
	bmp_common_hdr(s, BMP_VERSION_3, BMP_TYPE_ROUTE_MIRRORING);
	bmp_per_peer_hdr(s, bmp->targets->bgp, peer, 0, BMP_PEER_TYPE_GLOBAL_INSTANCE, 0, &bmq->tv);
...
afibreak:
	...
	if (bpi && CHECK_FLAG(bpi->flags, BGP_PATH_SELECTED)
		...
		bmp_monitor(bmp, bpi->peer, 0,
			    BMP_PEER_TYPE_LOC_RIB_INSTANCE, bmp_get_peer_distinguisher(bmp, afi), bn_p,
			    prd, bpi->attr, afi, safi, bpi->rib_uptime);
	}
	...
	if (bpi && CHECK_FLAG(bpi->flags, BGP_PATH_VALID)
		...
		bmp_monitor(bmp, bpi->peer, BMP_PEER_FLAG_L,
			    BMP_PEER_TYPE_GLOBAL_INSTANCE, 0, bn_p,
...
static struct bmp_queue_entry *bmp_pull_from_queue(struct bmp_qlist_head* list, struct bmp_qhash_head* hash, struct bmp_queue_entry** queuepos_ptr)
{
...
static inline struct bmp_queue_entry *bmp_pull(struct bmp* bmp) {
	return bmp_pull_from_queue(&bmp->targets->updlist, &bmp->targets->updhash, &bmp->queuepos);
}

static inline struct bmp_queue_entry *bmp_pull_locrib(struct bmp* bmp) {
	return bmp_pull_from_queue(&bmp->targets->locupdlist, &bmp->targets->locupdhash, &bmp->locrib_queuepos);
}
...
static bool bmp_wrqueue_locrib(struct bmp* bmp, struct pullwr* pullwr) {
	...
	case BMP_AFI_SYNC:
		...
		// TODO BMP_MON_LOCRIB check if this is true after implementing LOCRIB syncing
		/* currently syncing & haven't reached this prefix yet
		 * => it'll be sent as part of the table sync, no need here */
		zlog_info("bmp: skipping direct monitor msg bc will be sent with sync :)");
		goto out;
	case BMP_AFI_LIVE:
		break;
	}
	...
	if (is_vpn) {
		prd = &bqe->rd;
		bn = bgp_afi_node_lookup(bmp->targets->bgp->rib[afi][safi], afi, safi, &bqe->p, &bqe->rd);
	}
	...
	bmp_monitor(bmp, peer, 0, BMP_PEER_TYPE_LOC_RIB_INSTANCE, bmp_get_peer_distinguisher(bmp, afi),
...
static void bmp_stats(struct thread *thread)
{
	...
	for (ALL_LIST_ELEMENTS_RO(bt->bgp->peer, node, peer)) {
		...
		bmp_per_peer_hdr(s, bt->bgp, peer, 0, BMP_PEER_TYPE_GLOBAL_INSTANCE, 0, &tv);
      ...
      )
{
	...
	if (policy[0] == 'l') {
		...
		vty_out(vty, "changing loc rib monitoring config for this target\n");
	} else if (policy[1] == 'r')
	...
	frr_each(bmp_bgph, &bmp_bgph, bmpbgp) {
		...
		frr_each(bmp_targets, &bmpbgp->targets, bt) {
			...
			FOREACH_AFI_SAFI (afi, safi) {
				...
				const char* pre_str = afimon_flag & BMP_MON_PREPOLICY ? "pre-policy " : "";
				const char* post_str = afimon_flag & BMP_MON_POSTPOLICY ? "post-policy " : "";
				const char* locrib_str = afimon_flag & BMP_MON_LOC_RIB ? "loc-rib" : "";

				vty_out(vty, "    Route Monitoring %s %s %s%s%s\n",
					afi2str(afi), safi2str(safi), pre_str, post_str, locrib_str);
			}
...
static int bmp_route_update(struct bgp * bgp, afi_t afi, safi_t safi,
			    struct bgp_dest *bn, struct bgp_path_info* updated_route, bool withdraw) {

	zlog_info("bmp: got route update (%s) ! vrf_id=%d, afi/safi=%s, dest=%pRN, bpi=%pRN",
		  withdraw ? "withdraw" : "update", bgp->vrf_id, get_afi_safi_str(afi, safi, false), bn, updated_route->net);
	...
	frr_each(bmp_targets, &bmpbgp->targets, bt) {
		...
		FOREACH_AFI_SAFI(afi_iter, safi_iter) {
			if (bt->afimon[afi_iter][safi_iter])
				zlog_info("bmp: afi/safi=%s, flag=%d", get_afi_safi_str(afi_iter, safi_iter, false), bt->afimon[afi_iter][safi_iter]);
		};
		...
		if (bt->afimon[afi][safi] & BMP_MON_LOC_RIB) {
			...
			if (bqe) {
				...
				zlog_info("bmp: prefix not registered in queue yet");
				bqe = XMALLOC(MTYPE_BMP_QUEUE, sizeof(*bqe));
