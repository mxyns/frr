...
static inline bool
bmp_monitor_rib_out_walk(struct bmp *bmp, afi_t afi, safi_t safi,
			 struct bgp_dest *dest, struct attr *attr,
			 struct prefix_rd *prd,
			 int (*cb)(struct update_group *updgrd, void *ctx))
{
	bool written = false;
	struct rib_out_updgrp_walkctx walkctx = {.bmp = bmp,
						 .dest = dest,
						 .attr = attr,
						 .prd = prd,
						 .written_ref = &written};

	update_group_af_walk(bmp->targets->bgp, afi, safi, cb,
			     (void *)&walkctx);
						 ...
						 void *hidden_ctx)
{
	...
	struct rib_out_updgrp_walkctx *ctx =
		(struct rib_out_updgrp_walkctx *)hidden_ctx;

	struct update_subgroup *subgrp;
	struct peer_af *paf;
	UPDGRP_FOREACH_SUBGRP (updgrp, subgrp) {
		// TODO filter out who has this prefix in rib-out pre
...
static inline bool bmp_monitor_rib_out_pre_walk(struct bmp *bmp, afi_t afi,
						...
						struct prefix_rd *prd)
{
	return bmp_monitor_rib_out_walk(bmp, afi, safi, dest, attr, prd,
						  ...
						  void *hidden_ctx)
{
	...
	UPDGRP_FOREACH_SUBGRP (updgrp, subgrp) {
		...
		advertised_attr = adj->adv && adj->adv->baa
					  ? adj->adv->baa->attr
					  : adj->attr;
		...
		SUBGRP_FOREACH_PEER (subgrp, paf) {
			bmp_monitor(ctx->bmp, PAF_PEER(paf),
				    ...
				    BMP_PEER_TYPE_GLOBAL_INSTANCE,
				    &ctx->dest->p, ctx->prd, advertised_attr,
				    SUBGRP_AFI(subgrp), SUBGRP_SAFI(subgrp),
				    monotime(NULL));
...
static inline bool bmp_monitor_rib_out_post_walk(struct bmp *bmp, afi_t afi,
						 ...
						 struct prefix_rd *prd)
{
	return bmp_monitor_rib_out_walk(bmp, afi, safi, dest, NULL, prd,
...
afibreak:
	...
	do {
		...
		if (bmp->targets->afimon[afi][safi] & BMP_MON_IN_POSTPOLICY ||
		    ...
		    bmp->targets->afimon[afi][safi] & BMP_MON_OUT_PREPOLICY ||
		    bmp->targets->afimon[afi][safi] & BMP_MON_OUT_POSTPOLICY) {
	...
	if (bpi && CHECK_FLAG(bpi->flags, BGP_PATH_SELECTED) &&
	    CHECK_FLAG(bmp->targets->afimon[afi][safi],
		       BMP_MON_OUT_PREPOLICY)) {
		written |= bmp_monitor_rib_out_pre_walk(bmp, afi, safi, bn,
							bpi->attr, prd);
	}
	...
	if (bpi && CHECK_FLAG(bpi->flags, BGP_PATH_SELECTED) &&
	    CHECK_FLAG(bmp->targets->afimon[afi][safi],
		       BMP_MON_OUT_POSTPOLICY)) {
		written |=
			bmp_monitor_rib_out_post_walk(bmp, afi, safi, bn, prd);
	}
 ...
 */
static inline struct bmp_queue_entry *bmp_pull_locrib_riboutpre(struct bmp *bmp)
{
 ...
 */
static inline struct bmp_queue_entry *bmp_pull_riboutpost(struct bmp *bmp)
{
 ...
 */
static bool bmp_wrqueue_locrib_riboutpre(struct bmp *bmp, struct pullwr *pullwr)
{
	struct bmp_queue_entry *bqe;
	struct peer *peer;
	struct bgp_dest *bn = NULL;
	...
	bqe = bmp_pull_locrib_riboutpre(bmp);
	if (!bqe)
		return false;
	...
	bool locrib_mon_conf =
		CHECK_FLAG(bmp->targets->afimon[afi][safi], BMP_MON_LOC_RIB);
	bool ribout_pre_conf = CHECK_FLAG(bmp->targets->afimon[afi][safi],
					  BMP_MON_OUT_PREPOLICY);
	if (!locrib_mon_conf && !ribout_pre_conf)
		goto out;
	...
	/* if rib-out pre-policy is configured and update group for this AF
	 * exists we send messages to all peers in the group about the updated
	 * prefix in rib-out-pre
	 */
	if (ribout_pre_conf &&
	    bmp->targets->bgp->update_groups[afindex(afi, safi)]) {
		written |= bmp_monitor_rib_out_pre_walk(
			bmp, afi, safi, bn, bpi ? bpi->attr : NULL, prd);
	}
 ...
 */
static bool bmp_wrqueue_riboutpost(struct bmp *bmp, struct pullwr *pullwr)
{
	struct bmp_queue_entry *bqe;
	struct peer *peer;
	struct bgp_dest *bn = NULL;
	...
	advertised_attr = adj ? (adj->adv && adj->adv->baa ? adj->adv->baa->attr
							   : adj->attr)
			      : NULL;

	bmp_monitor(bmp, peer, BMP_PEER_FLAG_L | BMP_PEER_FLAG_O,
		    BMP_PEER_TYPE_GLOBAL_INSTANCE, &bqe->p, prd,
		    advertised_attr, afi, safi, monotime(NULL));
...
static void bmp_wrfill(struct bmp *bmp, struct pullwr *pullwr)
{
	...
	case BMP_Run:
		...
		if (bmp_wrqueue_in(bmp, pullwr))
			break;
		if (bmp_wrqueue_locrib_riboutpre(bmp, pullwr))
			break;
		if (bmp_wrqueue_riboutpost(bmp, pullwr))
			break;
 ...
 */
static int bmp_process_ribin(struct bgp *bgp, afi_t afi, safi_t safi,
			     struct bgp_dest *bn, struct peer *peer,
			     bool withdraw)
{
	struct bmp_bgp *bmpbgp = bmp_bgp_find(peer->bgp);
	struct bmp_targets *bt;
	...
	if (frrtrace_enabled(frr_bgp, bmp_process_ribin)) {
		char pfxprint[PREFIX2STR_BUFFER];

		prefix2str(&bn->p, pfxprint, sizeof(pfxprint));
		frrtrace(5, frr_bgp, bmp_process_ribin, peer, pfxprint, afi,
			 safi, withdraw);
	}
	...
	frr_each(bmp_targets, &bmpbgp->targets, bt) {
		if (!CHECK_FLAG(bt->afimon[afi][safi],
				BMP_MON_IN_PREPOLICY | BMP_MON_IN_POSTPOLICY))
			continue;
...
static void bmp_close(struct bmp *bmp)
{
	...
	while ((bqe = bmp_pull_ribin(bmp)))
		if (!bqe->refcount)
			XFREE(MTYPE_BMP_QUEUE, bqe);
	while ((bqe = bmp_pull_locrib_riboutpre(bmp)))
		if (!bqe->refcount)
			XFREE(MTYPE_BMP_QUEUE, bqe);
	while ((bqe = bmp_pull_riboutpost(bmp)))
		if (!bqe->refcount)
			XFREE(MTYPE_BMP_QUEUE, bqe);
		...
		frr_each(bmp_targets, &bmpbgp->targets, bt) {
			...
			FOREACH_AFI_SAFI (afi, safi) {
				...
				const char *out_pre_str =
					afimon_flag & BMP_MON_OUT_PREPOLICY
						? "rib-out pre-policy "
						: "";
				const char *out_post_str =
					afimon_flag & BMP_MON_OUT_POSTPOLICY
						? "rib-out post-policy"
						: "";
			    ...
			    struct bgp_path_info *new_route)
{
	...
	frr_each (bmp_targets, &bmpbgp->targets, bt) {
		if (!CHECK_FLAG(bt->afimon[afi][safi],
				BMP_MON_LOC_RIB | BMP_MON_OUT_PREPOLICY))
			continue;
			       ...
			       bool withdraw)
{
	...
	zlog_info("adj out changed for dest=%pRN in subgrp=%" PRIu64
		  " (%d peers, %d adj), attr=%p, path=%p, type=%s",
		  dest, subgrp->id, subgrp->peer_count, subgrp->pscount, attr,
		  path, withdraw ? "withdraw" : "update");
		...
		frr_each (bmp_targets, &bmpbgp->targets, bt) {
			if (bt->afimon[afi][safi] & BMP_MON_OUT_POSTPOLICY) {
				...
				struct bmp_queue_entry *last_item =
					bmp_process_one(
						bt, &bt->mon_out_updhash,
						&bt->mon_out_updlist, NULL, afi,
						safi, dest, peer);
...
static int bgp_bmp_module_init(void)
{
	...
	hook_register(bgp_process, bmp_process_ribin);
