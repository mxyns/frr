			 ...
			 bool post_policy, bool withdraw, const char *caller)
{
	...
	if (post_policy) {
		hook_call(bgp_adj_out_updated, dest, subgrp, attr, NULL, true,
			  withdraw);
		return;
	}
	 ...
	 */
	bool pre_check = subgroup_announce_check(dest, path, subgrp, bgp_dest_get_prefix(dest),
						 &dummy_attr, NULL,
